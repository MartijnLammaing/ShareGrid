...
| **3** | Multiple hosts and users. Session reservation (1 user per host per session). Up to `SHAREGRID_MAX_SESSIONS` concurrent sessions per host. | Router gains per-host availability tracking (`availableSlots`/`totalSlots`) and handles dedicated `host_status_update` messages for immediate load signalling. LLMHost Session Manager becomes a capacity counter; InferenceProxy becomes slot-ID-aware (`id_slot` injection + per-slot KV-cache flush); RouterClient reports load on every session open/close and in every heartbeat. LLMUser Model Registry adds availability-aware host selection and aggregated slot metadata; Host Session Pool supports multiple concurrent sessions with conversation affinity (sequential turns always reuse the same session). |
| **4** | macOS native deployment mode with Metal GPU acceleration. | LLMHost gains an Apple Silicon native launch path: `LLAMA_TAG` file pins the `llama.cpp` version; Dockerfile Stage 1 reads it. `SHAREGRID_LLAMA_BINARY` and `SHAREGRID_SANDBOX_PROFILE` env vars make the inference binary path and sandbox profile configurable. `macos-native/` directory provides `setup.sh` (builds `llama-server` with Metal), `sandbox.sb` (SBPL hardening profile), `macos-run.sh` (native launch script), and `README.md`. `start-dev.sh` gains `--macos-host` flag. Node.js application code is unchanged; only the launch path differs. → [Phase 4 completion summary](phase_4_summary.md) |
