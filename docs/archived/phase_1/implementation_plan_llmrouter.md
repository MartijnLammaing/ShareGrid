...
| 8 | *(placeholder — reserved for numbering parity with LLMHost and LLMUser plans)* | 0 | — |
| 9 | Role-based access control (host/user URL split) | 9 | Phase 7 |
...
No tasks. This phase number is reserved to keep numbering consistent with the LLMHost and LLMUser plans, which each have a Phase 8 (prompt cancellation). The router required no equivalent changes.

---

## Phase 9 — Role-based access control (host/user URL split)

Introduces two role-specific secrets generated by the router on startup — a host secret and a user secret — embedded in separate URL sets. Every incoming connection must present the correct secret for its role before the router routes it to any handler. Two new items are added to `sharegrid-shared` as part of this phase (S-13 and S-14); they must be merged before any module-level Phase 9 work begins.

| #    | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | File / Location                                                   | Status |
|------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------|:------:|
| 9-1  | **(S-13)** Update `parseFingerprintFromUrl` in `sharegrid-shared/src/tls.ts` to also extract and return the `key` query parameter. Updated return type: `{ host: string, port: number, fingerprint: string, roleKey: string }`. Add `RoleKeyMissingError` (`"ROLE_KEY_MISSING"`) to `sharegrid-shared/src/errors.ts`; throw it if `key` is absent from the URL. Unit-test the new behaviour in `sharegrid-shared/tests/unit/tls.test.ts`: URL with valid `key` → extracted correctly; URL without `key` → throws `RoleKeyMissingError`. Bump `sharegrid-shared` with a patch version and update the `file:` dependency in `package.json`. | `sharegrid-shared/src/tls.ts`, `sharegrid-shared/src/errors.ts`, `sharegrid-shared/tests/unit/tls.test.ts` | `[x]`  |
| 9-2  | **(S-14)** Add `roleKey: string` field to `RegistrationPayload` and `HostListRequest` in `sharegrid-shared/src/protocol.ts`. This is the credential the client parsed from its URL and presents to the router for admission validation. No version bump required beyond the one in task 9-1. | `sharegrid-shared/src/protocol.ts`                                | `[x]`  |
| 9-3  | Extend `createKeyAuthority` to generate a `hostSecret` and a `userSecret` on construction using `crypto.randomBytes(32).toString('base64url')`. Expose `getHostSecret(): string` and `getUserSecret(): string`. Neither secret is written to disk or logged. Update the `KeyAuthority` interface type accordingly. | `src/key-authority.ts`                                            | `[x]`  |
| 9-4  | Update the TLS Listener host registration handler (task 3C-4). After demuxing on `type: "register"`, extract `roleKey` from the `RegistrationPayload` and validate it against `keyAuthority.getHostSecret()`. Reject immediately — close the socket and log at `warn` level with no registry write — if `roleKey` is missing or does not match. | `src/tls-listener.ts`                                             | `[x]`  |
| 9-5  | Update the TLS Listener user handshake handler (task 3C-6). After demuxing on `type: "host_list_request"`, extract `roleKey` from the `HostListRequest` and validate it against `keyAuthority.getUserSecret()`. Reject immediately — close the socket and log at `warn` level — if `roleKey` is missing or does not match. | `src/tls-listener.ts`                                             | `[x]`  |
| 9-6  | Update `printStartupBanner` to accept `{ listenAddr: string, fingerprint: string, hostSecret: string, userSecret: string }`. Print two separate labelled URL blocks: **"HOST REGISTRATION URLs"** (each candidate endpoint URL appended with `?fp=<fingerprint>&key=<hostSecret>`) and **"USER ACCESS URLs"** (appended with `?fp=<fingerprint>&key=<userSecret>`), matching the format documented in `architecture_llmrouter.md` §7. Update `src/index.ts` to pass `keyAuthority.getHostSecret()` and `keyAuthority.getUserSecret()` to `printStartupBanner`. | `src/startup-banner.ts`, `src/index.ts`                           | `[x]`  |
| 9-7  | Unit-test `key-authority.ts` additions. Cases: `getHostSecret()` and `getUserSecret()` each return a non-empty base64url string; they return different values from each other; two separate `createKeyAuthority()` calls each produce a different `hostSecret` and a different `userSecret` (four distinct values total). | `tests/unit/key-authority.test.ts`                                | `[x]`  |
| 9-8  | Unit-test `tls-listener.ts` role validation (extend the existing test file). Cases for the host registration path: missing `roleKey` → socket closed, no `hostRegistry.add` call; `roleKey` matching the user secret instead of the host secret → socket closed; correct host secret → registration proceeds normally. Same three cases mirrored for the user host-list path. | `tests/unit/tls-listener.test.ts`                                 | `[x]`  |
| 9-9  | Update `startup-banner.ts` unit tests: output contains exactly two labelled blocks matching "HOST REGISTRATION URLs" and "USER ACCESS URLs"; host URLs contain `key=<hostSecret>` and user URLs contain `key=<userSecret>`; both sets also contain `fp=<fingerprint>` and exclude loopback addresses. Update integration tests: add `roleKey` to all `RegistrationPayload` and `HostListRequest` fixtures; add rejection test cases — connection with missing `roleKey` is closed before any registry change; connection with wrong-role `roleKey` is closed. | `tests/unit/startup-banner.test.ts`, `tests/integration/registration.test.ts`, `tests/integration/host-list.test.ts` | `[x]`  |
...
| 8     | *(placeholder)*                                | 0     | 0    | 0           | 0       | 0         |
| 9     | Role-based access control (host/user URL split)| 9     | 9    | 0           | 0       | 0         |
| —     | **Total**                                      | **58**| **58**| **0**      | **0**   | **0**     |
